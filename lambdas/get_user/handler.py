...
def main(event, _context):
    LOGGER.info("Processing get user request: %s", event.get("pathParameters"))

    try:
        user_id = event["pathParameters"]["userId"]
        service = MercadoService(DynamoDbMercadoRepository())
        return json_response(200, service.get_user_data(user_id))
    except Exception as error:  # noqa: BLE001
        LOGGER.exception("Unhandled error in get_user lambda")
